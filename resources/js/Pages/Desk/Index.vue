    ...
    <AuthenticatedLayout>
        <template #header>
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500">Scannez d’abord la carte de bibliothèque pour identifier l’étudiant.</p>
            </div>
        </template>
        ...
        <section class="dw-card p-5 sm:p-6">
            <form class="flex flex-col gap-3 sm:flex-row" @submit.prevent="findStudent">
                ...
                <button type="button" class="flex h-12 items-center justify-center gap-2 rounded-md border border-primary-300 px-5 text-sm font-bold text-primary-600 hover:bg-primary-50 dark:border-primary-800 dark:text-primary-300 dark:hover:bg-primary-950" @click="cameraTarget = 'student'">
        ...
        <section v-if="student" class="mt-6 grid gap-6 xl:grid-cols-[360px_1fr]">
            <article class="dw-card self-start overflow-hidden xl:sticky xl:top-24">
                ...
                <div class="space-y-4 p-5 text-sm">
                    ...
                    <div class="flex justify-between gap-4"><span class="text-slate-400">Mention</span><strong class="text-end">{{ student.mention?.name || '—' }}</strong></div>
                    <div class="flex justify-between gap-4"><span class="text-slate-400">Parcours</span><strong class="text-end">{{ student.academic_program?.name || student.program || '—' }}</strong></div>
                    <div class="flex justify-between gap-4"><span class="text-slate-400">Niveau</span><strong class="text-end">{{ student.academic_level?.name || student.level || '—' }}</strong></div>
                    <div class="flex justify-between gap-4"><span class="text-slate-400">Année universitaire</span><strong class="text-end">{{ student.academic_year || '—' }}</strong></div>
                    <div class="flex justify-between gap-4"><span class="text-slate-400">Carte physique</span><strong class="text-end text-xs" :class="student.cards?.length ? 'text-emerald-600' : 'text-amber-600'">{{ student.cards?.length ? 'Active' : 'Non délivrée' }}</strong></div>
                    <div class="flex justify-between"><span class="text-slate-400">Statut</span><span class="rounded-full px-2.5 py-1 text-xs font-bold" :class="student.status === 'active' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300'">{{ statusLabels[student.status] || student.status }}</span></div>
                </div>
            </article>
                ...
                <template v-else>
                    ...
                    <article v-else class="dw-card overflow-hidden">
                        <div class="border-b border-slate-100 p-5">
                            ...
                            <form v-if="!session.closed_at" class="mt-5 flex flex-col gap-2 sm:flex-row" @submit.prevent="addCopy"><input ref="copyInput" v-model="copyForm.barcode" class="dw-field flex-1" placeholder="Scanner le code de l’exemplaire…" autocomplete="off" /><button type="button" class="flex h-12 items-center justify-center gap-2 rounded-md border border-primary-300 px-4 text-sm font-bold text-primary-600 hover:bg-primary-50 dark:border-primary-800 dark:text-primary-300 dark:hover:bg-primary-950" @click="cameraTarget = 'copy'"><AppIcon name="scan" class="h-5 w-5" /> Caméra</button><button class="h-12 rounded-md bg-slate-800 px-5 text-sm font-bold text-white dark:bg-primary-600">Ajouter</button></form>
                    ...
                    <article class="dw-card overflow-hidden">
                        <div class="border-b border-slate-100 p-5">
                            ...
                            <form v-if="!loan" class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end" @submit.prevent="openLoan">
                                <div class="flex-1"><label class="mb-2 block text-xs font-bold uppercase text-slate-400">Date limite de retour</label><input v-model="loanOpenForm.due_at" type="date" class="dw-field" required /></div>
                                <button class="h-12 rounded-md bg-amber-600 px-5 text-sm font-bold text-white hover:bg-amber-700">Ouvrir un prêt</button>
                            </form>
                            <InputError class="mt-2" :message="loanOpenForm.errors.due_at || $page.props.errors?.loan" />

                            <template v-if="loan">
                                <div class="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-400"><span>{{ loan.loan_number }}</span><span>Retour avant le {{ new Date(loan.due_at).toLocaleDateString('fr-FR') }}</span></div>
                                <form class="mt-5 flex flex-col gap-2 sm:flex-row" @submit.prevent="addLoanCopy">
                                    ...
                                    <button type="button" class="flex h-12 items-center justify-center gap-2 rounded-md border border-amber-300 px-4 text-sm font-bold text-amber-600 hover:bg-amber-50 dark:border-amber-800 dark:text-amber-300" @click="cameraTarget = 'loan-copy'"><AppIcon name="scan" class="h-5 w-5" /> Caméra</button>
                                    <button class="h-12 rounded-md bg-amber-600 px-5 text-sm font-bold text-white">Prêter</button>
                                </form>
        ...
        <details class="dw-card group mt-6 overflow-hidden">
            ...
            <div v-if="recentActivity.length" class="divide-y divide-slate-100 border-t border-slate-100 dark:divide-slate-800 dark:border-slate-800">
                <div v-for="activity in recentActivity" :key="activity.key" class="grid gap-3 p-4 sm:grid-cols-[44px_minmax(180px,0.8fr)_minmax(220px,1.4fr)_auto] sm:items-center sm:px-5">
                    ...
                    <div v-if="activity.book" class="min-w-0 sm:max-w-80 sm:text-end"><p class="truncate text-sm text-slate-600">{{ activity.book }}</p><p class="mt-1 font-mono text-xs text-slate-400">{{ activity.inventory_number }}</p></div>
                </div>
            </div>
