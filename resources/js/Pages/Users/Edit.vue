...
import { ref } from "vue";
const props = defineProps<{ managedUser: any; roles: string[] }>();
const form = useForm({
...
<template>
    <Head :title="`Modifier ${managedUser.name}`" /><AuthenticatedLayout
        ><template #header
            ><div>
                <Link
                ...
                <p class="dw-page-description">
                    Mettez à jour son identité, son rôle ou son accès.
                </p>
            </div></template
        >
        <form
        ...
        >
            <div class="grid gap-5 md:grid-cols-2">
                <div>
                    ...
                    ><input v-model="form.name" class="dw-field" /><InputError
                ...
                <div>
                    <label class="mb-2 block text-sm">Statut</label
                    ><select v-model="form.is_active" class="dw-field">
                        <option :value="true">Actif</option>
                        <option :value="false">Suspendu</option></select
