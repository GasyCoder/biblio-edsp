        ...
        <section class="mt-6 space-y-3">
            <details v-for="student in studentGroups.data" :key="student.id" class="dw-card group overflow-hidden" :open="student.active_visits_count > 0">
                <summary class="flex cursor-pointer list-none flex-wrap items-center gap-4 p-4 marker:hidden sm:p-5">
                    ...
                    <img v-if="student.photo_url" :src="student.photo_url" class="h-12 w-10 shrink-0 rounded object-cover" alt=""/><span v-else class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary-50 text-xs font-bold text-primary-700">{{ student.first_name[0] }}{{ student.last_name[0] }}</span>
                    <span class="min-w-0 flex-1"><span class="flex flex-wrap items-center gap-2"><strong class="truncate text-sm text-slate-700 dark:text-slate-200">{{ student.last_name }} {{ student.first_name }}</strong><span v-if="student.active_visits_count" class="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold uppercase text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">Présent maintenant</span></span><span class="mt-1 block font-mono text-xs text-primary-600">{{ student.registration_number }}</span></span>
                    <span class="hidden text-end sm:block"><strong class="block text-sm text-slate-700 dark:text-slate-200">{{ student.matching_visits_count }} passage(s)</strong><span class="mt-1 block text-xs text-slate-400">Dernier : {{ new Date(student.latest_visit_at).toLocaleDateString('fr-FR') }}</span></span>
                    ...
                    <AppIcon name="chevron-down" class="h-5 w-5 shrink-0 text-slate-400 transition group-open:rotate-180"/>
                </summary>
                ...
                <div class="border-t border-gray-200 bg-gray-50/60 dark:border-gray-900 dark:bg-gray-1000/40">
                    <div class="grid grid-cols-[18px_1fr] gap-x-3 p-4 sm:p-5">
                        <template v-for="(visit, index) in student.visits" :key="visit.id">
                            ...
                            <div class="mb-4 grid gap-3 rounded-md border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950 sm:grid-cols-[1fr_1fr_110px_110px] sm:items-center"><div><p class="font-mono text-xs font-bold text-primary-600">{{ visit.visit_number }}</p><p class="mt-1 text-xs text-slate-400">Entrée : {{ new Date(visit.checked_in_at).toLocaleString('fr-FR') }}</p></div><div><p v-if="visit.checked_out_at" class="text-xs text-slate-500">Sortie : {{ new Date(visit.checked_out_at).toLocaleString('fr-FR') }}</p><span v-else class="text-xs font-bold text-emerald-600">Toujours présent</span></div><span class="text-xs font-bold text-slate-600 dark:text-slate-300">{{ duration(visit) }}</span><span class="text-xs text-slate-500">{{ visit.consultation_sessions_count }} consultation(s)</span></div>
                        </template>
                    </div>
                </div>
            </details>
            <div v-if="!studentGroups.data.length" class="dw-card p-12 text-center"><AppIcon name="visits" class="mx-auto h-10 w-10 text-slate-300"/><p class="mt-3 text-sm font-bold text-slate-600">Aucune présence trouvée</p><p class="mt-1 text-xs text-slate-400">Modifiez les filtres pour afficher d’autres passages.</p></div>
        </section>

        <div v-if="studentGroups.links?.length > 3" class="mt-5 flex flex-wrap justify-center gap-1"><Link v-for="link in studentGroups.links" :key="link.label" :href="link.url || '#'" class="rounded-md border px-3 py-2 text-xs" :class="link.active ? 'border-primary-600 bg-primary-600 text-white' : link.url ? 'border-gray-300 bg-white text-slate-600 dark:border-gray-800 dark:bg-gray-950' : 'pointer-events-none border-gray-200 text-slate-300'" v-html="link.label"/></div>
    </AuthenticatedLayout>
</template>
