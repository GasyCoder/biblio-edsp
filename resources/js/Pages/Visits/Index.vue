                ...
                <div class="border-t border-gray-200 bg-gray-50/60 dark:border-gray-900 dark:bg-gray-1000/40">
                    <div class="grid grid-cols-[18px_1fr] gap-x-3 p-4 sm:p-5">
                        <template v-for="(visit, index) in student.visits" :key="visit.id">
                            ...
                            <div class="mb-4 grid gap-3 rounded-md border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-950 sm:grid-cols-[1fr_1fr_110px_110px] sm:items-center"><div><p class="font-mono text-xs font-bold text-primary-600">{{ visit.visit_number }}</p><p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Entrée : {{ new Date(visit.checked_in_at).toLocaleString('fr-FR') }}</p></div><div><p v-if="visit.checked_out_at" class="text-xs text-slate-500">Sortie : {{ new Date(visit.checked_out_at).toLocaleString('fr-FR') }}</p><span v-else class="text-xs font-bold text-emerald-600">Toujours présent</span></div><span class="text-xs font-bold text-slate-600 dark:text-slate-300">{{ duration(visit) }}</span><span class="text-xs text-slate-500">{{ visit.consultation_sessions_count }} consultation(s)</span></div>
                        </template>
                    </div>
                </div>
            </details>
