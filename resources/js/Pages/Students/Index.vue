...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <section class="dw-card overflow-hidden">
            ...
            <div class="overflow-x-auto"><table class="w-full min-w-[850px] text-start text-sm"><thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-400"><tr><th class="px-5 py-3 text-start">Étudiant</th><th class="px-5 py-3 text-start">N° interne</th><th class="px-5 py-3 text-start">Matricule</th><th class="px-5 py-3 text-start">Niveau / parcours</th><th class="px-5 py-3 text-start">Statut</th></tr></thead><tbody class="divide-y divide-slate-100"><tr v-for="student in students.data" :key="student.id" class="hover:bg-slate-50/70"><td class="px-5 py-4 font-semibold text-slate-700">{{ student.last_name }} {{ student.first_name }}</td><td class="px-5 py-4 font-mono text-xs text-primary-700">{{ student.registration_number }}</td><td class="px-5 py-4 text-slate-500">{{ student.academic_number || '—' }}</td><td class="px-5 py-4 text-slate-500">{{ [student.level, student.program].filter(Boolean).join(' · ') || '—' }}</td><td class="px-5 py-4"><span :class="student.status === 'active' ? 'bg-emerald-50 text-emerald-700' : student.status === 'suspended' ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-600'" class="rounded-full px-2.5 py-1 text-xs font-bold">{{ statusLabel[student.status] }}</span></td></tr><tr v-if="!students.data.length"><td colspan="5" class="px-5 py-16 text-center text-sm text-slate-400">Aucun étudiant trouvé.</td></tr></tbody></table></div>
