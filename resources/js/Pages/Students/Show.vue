...
<template>
    ...
    /><AuthenticatedLayout
        ...
        <div class="mt-6 grid gap-6 xl:grid-cols-2">
            <section class="dw-card overflow-hidden">
                <div class="flex items-center justify-between gap-4 border-b border-gray-200 p-5 dark:border-gray-900"><div><p class="text-xs font-bold uppercase tracking-widest text-primary-600">Lecture sur place</p><h2 class="mt-1 font-heading text-lg font-bold text-slate-800">Livres consultés</h2><p class="mt-1 text-xs text-slate-400">{{ consultedBooks.total }} consultation(s) enregistrée(s)</p></div><span class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary-50 text-primary-600 dark:bg-primary-950"><AppIcon name="books" class="h-5 w-5"/></span></div>
                <div v-if="consultedBooks.data.length" class="divide-y divide-gray-200 dark:divide-gray-900">
                    <div v-for="item in consultedBooks.data" :key="item.id" class="flex gap-4 p-4 sm:p-5"><img v-if="item.copy.book.cover_url" :src="item.copy.book.cover_url" :alt="`Couverture de ${item.copy.book.title}`" class="h-20 w-14 shrink-0 rounded object-cover shadow-sm"/><span v-else class="flex h-20 w-14 shrink-0 items-center justify-center rounded border border-dashed border-gray-300 bg-gray-50 text-primary-500 dark:border-gray-800 dark:bg-gray-900"><AppIcon name="books" class="h-6 w-6"/></span><div class="min-w-0 flex-1"><Link :href="route('books.show', item.copy.book.id)" class="line-clamp-2 text-sm font-bold text-slate-700 hover:text-primary-600 dark:text-slate-200">{{ item.copy.book.title }}</Link><p class="mt-1 font-mono text-xs text-primary-600">{{ item.copy.inventory_number }}</p><div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-400"><span>{{ new Date(item.scanned_at).toLocaleString('fr-FR') }}</span><span>·</span><span>{{ item.session.session_number }}</span></div></div><span class="h-fit rounded-full px-2.5 py-1 text-[11px] font-bold" :class="item.returned_at ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300'">{{ item.returned_at ? 'Restitué' : 'En consultation' }}</span></div>
                </div><div v-else class="p-10 text-center"><AppIcon name="books" class="mx-auto h-9 w-9 text-slate-300"/><p class="mt-3 text-sm text-slate-400">Aucun livre consulté.</p></div>
                <div v-if="consultedBooks.links?.length > 3" class="flex flex-wrap justify-center gap-1 border-t border-gray-200 p-4 dark:border-gray-900"><Link v-for="link in consultedBooks.links" :key="link.label" :href="link.url || '#'" class="rounded border px-2.5 py-1.5 text-xs" :class="link.active ? 'border-primary-600 bg-primary-600 text-white' : link.url ? 'border-gray-300 text-slate-500 dark:border-gray-800' : 'pointer-events-none border-gray-200 text-slate-300'" v-html="link.label"/></div>
            </section>

            <section class="dw-card overflow-hidden">
                <div class="flex items-center justify-between gap-4 border-b border-gray-200 p-5 dark:border-gray-900"><div><p class="text-xs font-bold uppercase tracking-widest text-amber-600">Prêt à domicile</p><h2 class="mt-1 font-heading text-lg font-bold text-slate-800">Livres empruntés</h2><p class="mt-1 text-xs text-slate-400">{{ borrowedBooks.total }} emprunt(s) enregistré(s)</p></div><span class="flex h-11 w-11 items-center justify-center rounded-lg bg-amber-50 text-amber-600 dark:bg-amber-950"><AppIcon name="loans" class="h-5 w-5"/></span></div>
                <div v-if="borrowedBooks.data.length" class="divide-y divide-gray-200 dark:divide-gray-900">
                    <div v-for="item in borrowedBooks.data" :key="item.id" class="flex gap-4 p-4 sm:p-5"><img v-if="item.copy.book.cover_url" :src="item.copy.book.cover_url" :alt="`Couverture de ${item.copy.book.title}`" class="h-20 w-14 shrink-0 rounded object-cover shadow-sm"/><span v-else class="flex h-20 w-14 shrink-0 items-center justify-center rounded border border-dashed border-gray-300 bg-gray-50 text-amber-500 dark:border-gray-800 dark:bg-gray-900"><AppIcon name="loans" class="h-6 w-6"/></span><div class="min-w-0 flex-1"><Link :href="route('books.show', item.copy.book.id)" class="line-clamp-2 text-sm font-bold text-slate-700 hover:text-primary-600 dark:text-slate-200">{{ item.copy.book.title }}</Link><p class="mt-1 font-mono text-xs text-amber-600">{{ item.copy.inventory_number }}</p><div class="mt-3 flex flex-wrap gap-2 text-xs text-slate-400"><span>Prêté le {{ new Date(item.loaned_at).toLocaleDateString('fr-FR') }}</span><span>·</span><span>Retour prévu {{ new Date(item.loan.due_at).toLocaleDateString('fr-FR') }}</span></div></div><span class="h-fit rounded-full px-2.5 py-1 text-[11px] font-bold" :class="item.returned_at ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : new Date(item.loan.due_at) < new Date() ? 'bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300' : 'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300'">{{ item.returned_at ? 'Rendu' : new Date(item.loan.due_at) < new Date() ? 'En retard' : 'Emprunté' }}</span></div>
                </div><div v-else class="p-10 text-center"><AppIcon name="loans" class="mx-auto h-9 w-9 text-slate-300"/><p class="mt-3 text-sm text-slate-400">Aucun livre emprunté.</p></div>
                <div v-if="borrowedBooks.links?.length > 3" class="flex flex-wrap justify-center gap-1 border-t border-gray-200 p-4 dark:border-gray-900"><Link v-for="link in borrowedBooks.links" :key="link.label" :href="link.url || '#'" class="rounded border px-2.5 py-1.5 text-xs" :class="link.active ? 'border-amber-600 bg-amber-600 text-white' : link.url ? 'border-gray-300 text-slate-500 dark:border-gray-800' : 'pointer-events-none border-gray-200 text-slate-300'" v-html="link.label"/></div>
            </section>
        </div>
        </AuthenticatedLayout
    >
