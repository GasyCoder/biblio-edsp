...
const allSelected = computed(
    () =>
        ...
        props.categories.every((item) => selected.value.includes(item.id)),
);
const toggleAll = () =>
...
const create = () =>
    form.post(route("categories.store"), { onSuccess: () => form.reset() });
const save = () =>
    editing.value &&
    router.patch(route("categories.update", editing.value.id), editing.value, {
        onSuccess: () => (editing.value = null),
    });
const remove = (item: any) =>
    confirm(`Supprimer la catégorie « ${item.name} » ?`) &&
    router.delete(route("categories.destroy", item.id));
const removeSelected = () =>
    selected.value.length &&
    confirm(`Supprimer ${selected.value.length} catégorie(s) ?`) &&
    router.delete(route("categories.destroy.bulk"), {
...
<template>
    <Head title="Catégories" /><AuthenticatedLayout>
        <template #header
            ><div
            ...
            >
                <div>
                    <Link
                        ...
                        >← Référentiels</Link
                    >
                    <h1 class="dw-page-title">Catégories</h1>
                    <p class="dw-page-description">
                        Classement des ouvrages et génération des préfixes
                        d’inventaire.
                    </p>
                </div>
            </div></template
        ...
        <div class="grid gap-6 xl:grid-cols-[360px_1fr]">
            <section v-if="canCreate" class="dw-card self-start p-5">
                <h2 class="text-lg font-bold">Nouvelle catégorie</h2>
                <form class="mt-5 space-y-4" @submit.prevent="create">
                    ...
                    <div>
                        <label class="mb-2 block text-sm">Code inventaire</label
                        ><input
                            v-model="form.inventory_code"
                            class="dw-field uppercase"
                            maxlength="10"
            ...
            <section class="dw-card overflow-hidden">
                ...
                >
                    <div>
                        <h2 class="text-lg font-bold">Liste des catégories</h2>
                        <p
                            class="mt-1 text-xs text-slate-500 dark:text-slate-400"
                        >
                            {{ categories.length }} résultat(s)
                        </p>
                    </div>
                    <button
                ...
                <div class="overflow-x-auto">
                    <table class="dw-table min-w-[700px]">
                        <thead>
                            <tr>
                                ...
                                <th class="p-4 text-start">Catégorie</th>
                                <th class="p-4 text-start">Code</th>
                            ...
                            <tr v-for="item in categories" :key="item.id">
                                ...
                                <td class="p-4">
                                    ...
                                    <div v-else>
                                        <Link
                                            ...
                                            "
                                            :title="`Voir les ouvrages de la catégorie ${item.name}`"
                                ...
                                <td class="p-4">
                                    <input
                                        ...
                                        v-model="editing.inventory_code"
                                        class="dw-field h-9 w-28 uppercase"
                                    /><span
                                        v-else
                                        class="font-mono text-xs font-bold text-primary-600"
                                ...
                                <td class="p-4 text-center">
                                    <Link
                                        ...
                                        "
                                        :aria-label="`Voir les ${item.books_count} ouvrages de ${item.name}`"
